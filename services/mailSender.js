...
const sendMail = async (req,res) => {
    const smtpConfiguration = await SmtpConfiguration.findOne({ idProject: req.body.idProject });
    if (!smtpConfiguration) {
        return res.status(404).json({message : "Smtp Configuration not found"});
    }
    const supportEmail  = await SupportEmail.findOne({ idSubProject: req.body.idSubProject });
    if (!supportEmail) {
