...
const generateUsername = async (firstname, lastname) => {
  ...
  let user;
  do {
    const randomDigitsOrLetters = Array.from(
      { length: 2 },
...
const resgisterWithUpload = async (req, res) => {
  try {
    ...
    // verification of the projects
    if (user.clients.length === 0 && user.projects.length === 0) {
      return res
        .status(402)
        .json({ error: "You must select at least one client" });
    } else if (user.clients.length !== 0) {
      user.clients.map(async (prj) => {
    ...
    // verification of the subProjects
    if (user.projects.length === 0 && user.clients.length === 0) {
      return res
        .status(403)
        .json({ error: "You must select at least one project" });
    } else if (user.projects.length !== 0) {
      user.projects.map(async (subPrj) => {
