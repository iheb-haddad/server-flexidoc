...
const checkIfSourcesExistInUrl = (currentUrl, sourceList) => {
  const sourcePattern = sourceList
...
const getMappingsByParameters = async (req, res) => {
  ...
  const mappings = allMappings.filter(
    (mapping) =>
      checkIfSourcesExistInUrl(currentUrl, mapping.idSource.keywords) &&
      mapping.idDocument.status === "public"
  );
  res.send(mappings);
};
...
const createMapping = async (req, res) => {
  ...
  if (!project) {
    res.status(404).json({ message: "Client non reconnu" });
    return;
  }
...
const createMappingWithUpload = async (req, res) => {
  ...
  const project = await Project.findOne({ name: mapping.client });
  if (!project) {
    res.status(404).json({ message: "Client non reconnu" });
    return;
  }
