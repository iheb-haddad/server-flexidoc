...
const createSubProject = async (req, res) => {
  ...
  if (existingSubProject) {
    return res
      .status(400)
      .json({
        message: "Project with this name already exists under this client",
      });
  }
...
const createSubProjectWithUpload = async (req, res) => {
  const subProject = req.body;
  const project = await Project.findOne({ name: subProject.client });
  if (!project) {
    return res.status(404).json({ message: "Client not found" });
  }
  const existingSubProject = await SubProject.findOne({
  ...
  if (user) {
    ...
    if (user.role === "admin") {
      try {
        ...
        res.status(403).json({ message: "Projects not found" });
      }
    } else if (user.role === "user") {
      let userSubprojects = [];
      for (let i = 0; i < user.subProjects.length; i++) {
        userSubprojects.push(
          await SubProject.findOne({ _id: user.subProjects[i] }).populate(
            "idProject"
          )
        );
      }
...
const getSubProject = async (req, res) => {
  ...
  const subProject = await SubProject.findOne({ _id: _id }).populate(
    "idProject"
  );
  if (!subProject) {
