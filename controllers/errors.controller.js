...
const createDocumentation =
  require("./additionalControllers").createDocumentation;
const createDocumentationWithUpload =
  require("./additionalControllers").createDocumentationWithUpload;
...
const createError = async (req, res) => {
  ...
  const documentation = await createDocumentation(error);
  if (!documentation) {
  ...
  const errorDocument = await Error.findOne({
    idDocumentation: error.idDocumentation,
    idSubProject: error.idSubProject,
  });
  if (errorDocument) {
...
const createErrorWithUpload = async (req, res) => {
  ...
  const documentation = await createDocumentationWithUpload(error);
  if (!documentation) {
    return res.status(404).json({ message: "Documentation not found" });
  }

  const project = await Project.findOne({ name: error.client });
  if (!project) {
    return res.status(404).json({ message: "Client not found" });
  }
  ...
  const errorDocument = await Error.findOne({
    idDocumentation: documentation._id,
